...
struct Follower {
	...
	float step(float* left, float* right, float recovery) {
		...
		if (value >= level) {
			...
			level -= level / (engineGetSampleRate() * recovery);
		}
...
void DuckModule::step() {
	auto amount = clamp2(follower.step(&inputs[LEFT_OVER_AUDIO].value, &inputs[RIGHT_OVER_AUDIO].value, params[RECOVERY_PARAM].value) * params[AMOUNT_PARAM].value, 0.f, 1.f);
...
struct DuckModuleWidget : ModuleWidget {
	DuckModuleWidget(DuckModule *module) : ModuleWidget(module) {
		...
		addParam(ParamWidget::create<Davies1900hLargeWhiteKnob>(Vec(10, 65), module, DuckModule::AMOUNT_PARAM, 0.0, 10.0, 5.0));
